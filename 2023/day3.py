...
def add_gear(number, key):
    print('Adding gear part {} for {}'.format(number, key))
    ...
    for i, c in enumerate(l):
        if c.isnumeric():
            ...
            if part_number:
                ...
                if i == (len(l) - 1):
                    if gear:
                        add_gear(number, key)
                        gear = False
                    print('Adding {} to line_sum'.format(number))
                    line_sum += int(number)
            else:
                # check left if not at the start
                if i > 0:
                    if not l[i-1].isnumeric() and l[i-1] != ".":
                        print('Symbol {} left makes it part number'.format(l[i-1]))
                        part_number = True
                        if l[i-1] == "*":
                            gear = True
                ...
                if i < len(l) - 1:
                    if not l[i+1].isnumeric() and l[i+1] != ".":
                        print('Symbol {} right makes it part number'.format(l[i+1]))
                        part_number = True
                        if l[i+1] == "*":
                            gear = True
                            key = '{}-{}'.format(lnumber, i+1)
                ...
                if prev:
                    if i > 0:
                        if not prev[i-1].isnumeric() and prev[i-1] != ".":
                            print('Symbol {} top left makes it part number'.format(prev[i-1]))
                            part_number = True
                            if prev[i-1] == "*":
                                gear = True
                        ...
                        if not prev[i].isnumeric() and prev[i] != ".":
                            print('Symbol {} top makes it part number'.format(prev[i]))
                            part_number = True
                            if prev[i] == "*":
                                gear = True
                        ...
                        if i < len(l) - 1 and not prev[i+1].isnumeric() and prev[i+1] != ".":
                            print('Symbol {} top right makes it part number'.format(prev[i+1]))
                            part_number = True
                            if prev[i+1] == "*":
                                gear = True
                                key = '{}-{}'.format(lnumber-1, i+1)
                ...
                if nxt:
                    if i > 0:
                        if not nxt[i-1].isnumeric() and nxt[i-1] != ".":
                            print('Symbol {} bottom left makes it part number'.format(nxt[i-1]))
                            part_number = True
                            if nxt[i-1] == "*":
                                gear = True
                                key = '{}-{}'.format(lnumber+1, i-1)
                        ...
                        if not nxt[i].isnumeric() and nxt[i] != ".":
                            print('Symbol {} bottom makes it part number'.format(nxt[i]))
                            part_number = True
                            if nxt[i] == "*":
                                gear = True
                                key = '{}-{}'.format(lnumber+1, i)
                        ...
                        if i < len(l) - 1 and not nxt[i+1].isnumeric() and nxt[i+1] != ".":
                            print('Symbol {} bottom right makes it part number'.format(nxt[i+1]))
                            part_number = True
                            if nxt[i+1] == "*":
                                gear = True
                                key = '{}-{}'.format(lnumber+1, i+1)
            ...
            if number and part_number:
                ...
                number = ""
                part_number = False
    print('Line sum: {}'.format(line_sum))    
    return [line_sum]
...
for g in gears:
    if gears[g][0] and gears[g][1]:
        ...
        gear_sum += ratio
        print('Gear with key {} {}:{} has ratio {}.'.format(g, gears[g][0], gears[g][1], ratio))
print(gear_sum)